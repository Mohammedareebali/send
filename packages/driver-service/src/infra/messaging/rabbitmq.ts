...
export class RabbitMQService {
  ...
  private readonly exchange = 'driver-events';
  private readonly queue = 'driver-notifications';
    ...
    try {
      ...
      await this.channel.assertQueue(this.queue, { 
        ...
        arguments: {
          ...
          'x-dead-letter-routing-key': 'driver.dead-letter'
        }
      });
      await this.channel.bindQueue(this.queue, this.exchange, 'driver.*');
      ...
      await this.channel.bindQueue(`${this.queue}.dlq`, `${this.exchange}.dlx`, 'driver.dead-letter');
  ...
  async publishDriverEvent(event: DriverEvent): Promise<void> {
    ...
    try {
      ...
      console.error('Failed to publish driver event:', error);
      if (error instanceof Error && error.message.includes('channel closed')) {
        await this.connect();
        await this.publishDriverEvent(event);
      } else {
        throw error;
      }
    }
  ...
  async subscribeToDriverEvents(callback: (event: DriverEvent) => Promise<void>): Promise<void> {
    ...
    try {
      await this.channel.consume(this.queue, async (msg) => {
        ...
        try {
          const event = JSON.parse(msg.content.toString()) as DriverEvent;
        ...
        noAck: false,
        consumerTag: 'driver-service-consumer'
      });
    } catch (error) {
      console.error('Failed to subscribe to driver events:', error);
      if (error instanceof Error && error.message.includes('channel closed')) {
        await this.connect();
        await this.subscribeToDriverEvents(callback);
      } else {
        throw error;
      }
    }
